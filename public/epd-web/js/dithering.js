...
function processImageData(imageData, mode) {
  ...
  if (mode === 'sixColor') {
    ...
    // Initialize both buffers to 0xFF (white)
    blackWhiteData.fill(0xFF);
    redWhiteData.fill(0xFF);
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const closest = findClosestColor(r, g, b, 'threeColor');
        const byteIndex = y * byteWidth + Math.floor(x / 8);
        const bitIndex = 7 - (x % 8);

        if (closest.r === 255 && closest.g === 0 && closest.b === 0) {
          // RED: blackWhiteData bit is 1 (white), redWhiteData bit is 0 (red)
          blackWhiteData[byteIndex] |= (0x01 << bitIndex);
          redWhiteData[byteIndex] &= ~(0x01 << bitIndex);
        } else if (closest.r === 0 && closest.g === 0 && closest.b === 0) {
          // BLACK: blackWhiteData bit is 0 (black), redWhiteData bit is 1 (non-red)
          blackWhiteData[byteIndex] &= ~(0x01 << bitIndex);
          redWhiteData[byteIndex] |= (0x01 << bitIndex);
        } else {
          // WHITE: blackWhiteData bit is 1 (white), redWhiteData bit is 1 (non-red)
          blackWhiteData[byteIndex] |= (0x01 << bitIndex);
          redWhiteData[byteIndex] |= (0x01 << bitIndex);
        }
      }
